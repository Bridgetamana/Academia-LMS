...
const StudentSider = () => {
  ...
  const menuItems = [
    { label: 'Dashboard', icon: faTableColumns, href: '/student/dashboard', key: 'dashboard' },
    { label: 'Settings', icon: faGear, href: '/student/settings', key: 'settings' },
  ];

  return (
    <div className='w-64 shrink-0 flex flex-col bg-white border-r border-neutral-200 h-full overflow-y-auto'>
      <div className='p-5 mb-2'>
        <div className='flex items-center gap-3'>
          {isLoading ? (
            <>
              <div className='w-9 h-9 rounded-lg bg-neutral-100 animate-pulse shrink-0'></div>
              <div className='flex flex-col gap-1 w-full'>
                <div className='h-4 bg-neutral-100 animate-pulse rounded w-24'></div>
                <div className='h-3 bg-neutral-100 animate-pulse rounded w-16'></div>
              </div>
            </>
          ) : (
            <>
              <div className='w-9 h-9 rounded-lg bg-neutral-900 flex items-center justify-center overflow-hidden shrink-0'>
                {academyData?.logoUrl ? (
